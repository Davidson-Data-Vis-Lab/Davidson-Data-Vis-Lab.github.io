...
Kwasi and I took some time to sketch out the 3 different views we wanted and received feedback. Our tool, JITVis, visualizes each IR phase, like the Turbolizer and iongraph, but adds additional features that we thought the turbolizer lacked and recommended by Dr. Williams and Dr. Lim. I primarily worked on the dual view that does a phase-by-phase comparison.
...
Before I began coding, I researched some layouts specifically for the SoN style, as that was the first graph I tackled. I ended up using a Sugiyama style, which was a decent layout to start with, but quickly became unreadable as the SoN graphs grew larger. Nodes were clustered together or layered on top of each other. These graphs also have a lot of edges, so those were chaotic as well. Once I got a stable graph rendered, I added certain features that were recommended from my talks with Dr. Williams and Dr. Lim, and my notes on the Turbolizer. This included things like a search feature, color-coding the nodes, adding the relevant code, summarizing the entire phase, and adding information about the nodes upon clicking. This information was not too difficult to find, as it was just pulled from the JSON. With that, the first iteration of this view was done!
​
[old-SoN]
​
The next big step was implementing the CFG graphs. This is where I really began to research CFG implementation, specifically Spidermonkey’s iongraph. I looked at an article[link it] that explained how the layout worked and gave some pseudocode as well as their GitHub repository containing all the open-source code. Here, I used Claude to help me make sense of the algorithm and implement this specifically in JS because it was written in a different language. I had to change how the data was loaded in because Spidermonkey’s JSON is structurally different from the one I had been using, but rendering the edges was difficult. There were some floating edges and others that were disconnected. While it did take a day and a half to debug with my professor, the fix was actually pretty simple. I had an issue with actually drawing them as opposed to the algorithm not working.
​
[CFG-layout]
​
The last big push was refining the SoN’s layout, but we ended up deciding to use a different style. I still used a Sugiyama layout, but I added color and shape to the nodes and edges. This new style is based heavily on a tool called Seafoam [link], which displays GraalVM's Sea-of-Nodes graphs in an understandable and visually appealing way. It also shortened the edges so there was “less traffic”. Immediately, the first try was better than the previous, but I still ran into issues such as a missing node and edges for my test case. After more debugging and careful tracing of the JSON data, I figured out that a specific type of node and some mismatched labeling were mainly the cause of the missing node and edges.
